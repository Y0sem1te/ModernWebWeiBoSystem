...
//        System.out.println(res2);
    ...
    @GetMapping("/guan")
    public List<FollowAvatar> fetch_guan(@RequestParam("id") String username){
        List<FollowAvatar> res = userMapper.findGuan(username);
        for(int i=0; i<res.size(); i++){
            FollowAvatar fav = res.get(i);
            fav.setPhoto("http://127.0.0.1:8088/images/"+fav.getPhoto());
            res.set(i,fav);
        }
//        System.out.println(res);
        return res;
    }
    @GetMapping("/delete_weibo")
    public String delete_weibo(@RequestParam("id") int weiboid){
        int res = userMapper.deleteWeibo(weiboid);
//        System.out.println(res);
        if(res>0){
            return "success";
        }
