...
public interface UserMapper {
    ...
    @Select("SELECT f.followerid, u.photo FROM follow AS f JOIN `user` AS u ON u.id = f.followerid WHERE f.userid = #{username};")
    public List<FollowAvatar> findGuan(String username);

    @Delete("DELETE FROM weibo WHERE id = #{username};")
    public int deleteWeibo(int username);
}